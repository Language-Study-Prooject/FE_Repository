    ...
    >
      <Toolbar sx={{ px: { xs: 2, md: 3 } }}>
        ...
        >
          ...
          <Divider />
          {[
            { text: language === 'ko' ? '면접 연습 세션이 완료되었습니다.' : 'Interview session completed.', time: language === 'ko' ? '10분 전' : '10 min ago' },
            { text: language === 'ko' ? 'OPIC 모의고사 결과가 도착했습니다.' : 'OPIC test results arrived.', time: language === 'ko' ? '1시간 전' : '1 hour ago' },
            { text: language === 'ko' ? '새로운 학습 리포트가 생성되었습니다.' : 'New learning report generated.', time: language === 'ko' ? '어제' : 'Yesterday' },
          ].map((item, index) => (
            <MenuItem
              key={index}
